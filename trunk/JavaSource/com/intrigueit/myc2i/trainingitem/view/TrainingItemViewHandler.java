...
public class TrainingItemViewHandler extends BasePage implements Serializable {
  ...
  protected static final Logger logger = Logger.getLogger( TrainingItemViewHandler.class );
  ...
  @Autowired
	public TrainingItemViewHandler(TrainingItemService trainingItemService,VendorService vendorService) {
    ...
    try {
      ...
      if(recordList!=null && !recordList.isEmpty()) {
        ...
        while(list.hasNext()) {
          ...
          vList.add(new SelectItem(""+vendor.getVendorId(),vendor.getVendorName()));
        }      
      } 
      setVendorList(vList);
    } catch (Exception e) {     
  ...
  public void loadCategories() {    
    ...
    try {
      ...
      List<TrainingItem> recordList = trainingItemService.findByProperties(vendor, itemEnd);
     ...
     if (getVendorId()!=null && !getVendorId().isEmpty()) {
	   ...
	   try {  	       
  	    Date dt = new Date();           
  	    this.currentTrainingItem.setRecordLastUpdaterId(""+this.getMember().getMemberId());    
  	    this.currentTrainingItem.setRecordLastUpdatedDate(dt);
  	    this.setImagePath("");
  	    if (action.equals(ServiceConstants.ADD )) {
  	      this.currentTrainingItem.setRecordCreatorId(""+this.getMember().getMemberId());
	...
	public void preAddTrainingItem () {	  
	  ...
	  try {
	    if (preValidate()) {
	      ...
	      setSecHeaderMsg(this.getText("header_msg_training_material") + " " + this.getText("header_msg_add"));
	...
	public void addTrainingItem () {
	  ...
	  try {		  
		  ...
		  if(postValidate(this.currentTrainingItem)) {
		    this.trainingItemService.addTrainingItem(this.currentTrainingItem);
  			List<TrainingItem> itemList = (List<TrainingItem>) getTrainingItemLines().getWrappedData();
  			itemList.add(this.currentTrainingItem);
  			this.loadCategories();
  			logger.debug("Training item added: "+ this.currentTrainingItem.getItemId());
			}
		} catch (Exception e) {
		  setErrorMessage(this.getText("common_system_error"));
			logger.error(e.getMessage());
	...
	public void preUpdateTrainingItem() {
	  ...
	  if(this.getParameter(ServiceConstants.RECORD_ID)!=null) {
  		...
  		try {
  		  setErrorMessage("");
  		  this.currentTrainingItem = trainingItemService.loadById(Long.parseLong(recordId));
  		  this.setCommonData(ServiceConstants.UPDATE);
  		  setSecHeaderMsg(this.getText("header_msg_training_material") + " " + this.getText("header_msg_update"));
	...
	public void updateTrainingItem() {
	  ...
	  try {
	    ...
	    if(postValidate(this.currentTrainingItem)) {
  		  ...
  		  this.trainingItemService.updateTrainingItem(this.currentTrainingItem);
  		  putObjInList(rowIdx,this.currentTrainingItem);
  		  this.loadCategories();
  		  logger.debug("Training item updated: "+ this.currentTrainingItem.getItemId());
  		  setErrorMessage("");
  		}
		} catch (Exception e) {
		  setErrorMessage(this.getText("common_system_error"));
	...
	public void deleteTrainingItem() {
	  ...
	  if(this.getParameter(ServiceConstants.RECORD_ID)!=null) {
  		...
  		try {  			
				...
				trainingItemService.deleteTrainingItem(Long.parseLong(recordId));
				List<TrainingItem> itemList = (List<TrainingItem>) getTrainingItemLines().getWrappedData();
	...
	private void putObjInList(int idx,Object fetchObj) {
		try {
  	  List<TrainingItem> list = (List<TrainingItem>) getTrainingItemLines().getWrappedData();
  		list.remove(idx);
	...
	/**
	 * @param currentTrainingItem the currentTrainingItem to set
	 */
  ...
  public void setCategoryName(String categoryName) {
	...
	/**
	 * @param trainingItemLines the trainingItemLines to set
	 */
