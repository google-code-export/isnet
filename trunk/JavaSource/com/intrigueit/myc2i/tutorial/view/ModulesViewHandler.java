...
public class ModulesViewHandler extends BasePage implements Serializable {
		...
		try {
			...
			if (recordList != null && !recordList.isEmpty()) {
				...
				while (iterator.hasNext()) {
					TestTutorialDocument object = (TestTutorialDocument) iterator
							.next();
					if (object != null) {
	...
	public boolean preValidate() {
		...
		if (this.getDocumentId() == null || this.getDocumentId().isEmpty()) {
			errorMessage.append(this.getText("common_error_prefix"))
					.append(" ").append(
							this.getText("tutorial_module_doc_notselect"));
			flag = false;
		}
		if (this.getUserType() == null || this.getUserType().isEmpty()) {
			...
			errorMessage.append(this.getText("common_error_prefix"))
					.append(" ").append(
							this.getText("tutorial_module_user_notselect"));
			flag = false;
		}
		if (!flag)
	...
	public boolean postValidate(TestTutorialModules document) {
		...
		if (document == null) {
			...
			if (flag) {
				if (modulesService.isModuleExist(this.currentModules
						.getModulesId(), document.getDocumentId(),
						this.currentModules.getModuleName())) {
					errorMessage.append(this.getText("common_error_prefix"))
							.append(" ").append(
									this.getText("module_name_exist"));
					flag = false;
				}
			}
		}
	...
	public void getModulesByCritariya() {
		...
		try {
			...
			List<TestTutorialModules> recordList = modulesService
					.findByProperties(docTypeId, userId);
	...
	public void setCommonData(String action) {
		...
		if (getUserType() != null && !getUserType().isEmpty()) {
			this.currentModules.setMemberTypeIndicator(Long
					.parseLong(getUserType()));
		}
		try {
			Date dt = new Date();
	...
	public void preUpdateModules() {
		...
		if (this.getParameter(ServiceConstants.RECORD_ID) != null) {
			...
			try {
				this.currentModules = modulesService.loadById(Long
						.parseLong(recordId));
