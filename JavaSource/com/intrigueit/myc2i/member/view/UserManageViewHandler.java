...
public class UserManageViewHandler extends BasePage implements Serializable {	
  ...
  protected static final Logger logger = Logger.getLogger( UserManageViewHandler.class );
  ...
  /**
   * @param memberTypeHash the memberTypeHash to set
   */
  public void setMemberTypeHash(Hashtable<String, String> memberTypeHash) {
  ...
  @Autowired
	public UserManageViewHandler(MemberExService memberExService,MemberService memberService,
    ...
    try {           
       ...
       if (action.equals(ServiceConstants.ADD )) {
         if(this.getParameter(ServiceConstants.RECORD_TYPE)!=null) {
           this.setUserType(this.getParameter(ServiceConstants.RECORD_TYPE).toString());
           Hashtable<String, String> mType = this.getMemberTypeHash(); 
           if ( this.userType.equals(ServiceConstants.MENTOR)) {
             this.currentMember.setTypeId(Long.parseLong(mType.get(ServiceConstants.MENTOR)));
           } else if ( this.userType.equals(ServiceConstants.PROTEGE)) {
             this.currentMember.setTypeId(Long.parseLong(mType.get(ServiceConstants.PROTEGE)));
           } else if ( this.userType.equals(ServiceConstants.ADMIN)) {
             this.currentMember.setTypeId(Long.parseLong(mType.get(ServiceConstants.ADMIN)));
           } 
         }
         this.currentMember.setRecordCreatorId(""+this.getMember().getMemberId());
  ...
  private String parseMemberType ( String type ) {
    ...
    try {      
      ...
      setSecHeaderMsg(this.getText("header_msg_manage_user") + " " + this.getText("header_msg_add"));
  ...
  public void addUser () {
    ...
    try {     
      ...
      if(validate()) {
        if (validationPhase2()) {
          ...
          this.currentMember.setPassword(crp.getEncryptedText(this.currentMember.getPassword()));
          this.memberService.save(this.currentMember);
          List<Member> itemList = (List<Member>) getMemberLines().getWrappedData();
	...
	public void preUpdateUser () {
	  logger.debug(" Prepare user for update :: "+this.getParameter(ServiceConstants.RECORD_ID));
    if(this.getParameter(ServiceConstants.RECORD_ID)!=null) {
      String recordId = (String) this.getParameter(ServiceConstants.RECORD_ID);
      try {        
        if(this.getParameter(ServiceConstants.RECORD_TYPE)!=null) {
          String recordType = (String) this.getParameter(ServiceConstants.RECORD_TYPE);
          System.out.println(this.parseMemberType(recordType)+":::LLLL:::"+recordType);
