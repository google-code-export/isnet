...
public class QuestionAnsViewHandler extends BasePage implements Serializable {	
  ...
  public void getQuestionAnsByCategory() {
    ...
    try {
      ...
      List<TestTutorialQuestionAns> recordList = questionAnsService.findByProperties(moduleId);
      getQuestionAnsLines().setWrappedData(recordList);
    } catch (Exception e) {
      logger.error("Unable to load question and answer: "+e.getMessage());
    }
  }
  ...
  public void loadModuless() {
    ...
    try {
      ...
      if(recordList!=null && !recordList.isEmpty()) {
        ...
        while(list.hasNext()) {
          TestTutorialModules modules = (TestTutorialModules)list.next();
          vList.add(new SelectItem(""+modules.getModulesId(),modules.getModuleName()));
        }      
      } 
  ...
  public void loadCategories() {    
	...
	public Long getMaxPageNo() {
	  ...
	  try {
	    if (getModulesId()!=null && !getModulesId().isEmpty()) {
	      maxPageNo = this.questionAnsService.getMaxPageNo(Long.parseLong(getModulesId()));
	...
	public void preUpdateQuestionAns() {
	  ...
	  if(this.getParameter(ServiceConstants.RECORD_ID)!=null) {
  		...
  		try {
  		  setErrorMessage("");  	    
  		  this.currentQuestionAns = questionAnsService.loadById(Long.parseLong(recordId));
  		  this.setCommonData(ServiceConstants.UPDATE);
  		  setSecHeaderMsg(this.getText("header_msg_tutorial_qa") + " " + this.getText("header_msg_update"));
	...
	private void putObjInList(int idx,Object fetchObj) {
		try {
		  List<TestTutorialQuestionAns> list = (List<TestTutorialQuestionAns>) getQuestionAnsLines().getWrappedData();
		  list.remove(idx);
	...
	/**
	 * @param currentQuestionAns the currentQuestionAns to set
	 */
  ...
  public void setModulesList(List<SelectItem> modulesList) {
	...
	/**
	 * @param questionAnsLines the questionAnsLines to set
	 */
