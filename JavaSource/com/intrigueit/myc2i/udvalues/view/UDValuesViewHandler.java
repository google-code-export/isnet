...
public class UDValuesViewHandler extends BasePage implements Serializable {
  ...
  @Autowired
	...
	public void getUDValuesByCategory() {		
      ...
      List<UserDefinedValues> udValuesList = udvService.findByProperty("udValuesCategory", value);
  ...
  public void loadCategories() {	  
    ...
    try {
  	  Date dt = new Date();           
      this.currentUDValues.setRecordLastUpdaterId(""+this.getMember().getMemberId());    
      this.currentUDValues.setRecordLastUpdatedDate(dt);
  	  if (action.equals(ServiceConstants.ADD )) {
  	    this.currentUDValues.setRecordCreatorId(""+this.getMember().getMemberId());
    ...
    try {     
      ...
      setSecHeaderMsg(this.getText("header_msg_user_define_value") + " " + this.getText("header_msg_add"));
  ...
  public void addUDValues () {
    ...
    try {   
      if(validate()) {        
        ...
        logger.debug("User Define value added: "+ this.currentUDValues.getUdValuesId());
      }     
    } catch (Exception e) {
      setErrorMessage(this.getText("common_system_error"));
      logger.error(e.getMessage());
  ...
  public void preUpdateUDValues() {
    ...
    if(this.getParameter(ServiceConstants.RECORD_ID)!=null) {
      ...
      try {
        this.currentUDValues = udvService.loadById(Long.parseLong(recordId));
        this.setCommonData(ServiceConstants.UPDATE);
        setSecHeaderMsg(this.getText("header_msg_user_define_value") + " " + this.getText("header_msg_update"));
  ...
  public void updateUDValues() {
    ...
    try {      
      if(validate()) {
        ...
        logger.debug("User Define value updated: "+ this.currentUDValues.getUdValuesId());
      }
    } catch (Exception e) {
      setErrorMessage(this.getText("common_system_error"));
      logger.error(e.getMessage());
  ...
  public void deleteUDValues() {
    ...
    if(this.getParameter(ServiceConstants.RECORD_ID)!=null) {
      ...
      try {       
        ...
        logger.debug("User Define value deleted: "+ this.currentUDValues.getUdValuesId());
      } catch (Exception e) {
        setErrorMessage(this.getText("common_system_error"));
        logger.error(e.getMessage());
        e.printStackTrace();
	...
	private void putObjInList(int idx,Object fetchObj) {
		try {
		  List<UserDefinedValues> list = (List<UserDefinedValues>) getUdValuesLines().getWrappedData();
		  list.remove(idx);
	...
	/**
	 * @param currentUDValues the currentUDValues to set
	 */
	...
	/**
	 * @param udValuesLines the udValuesLines to set
	 */
